...
class LocalDataSource @Inject constructor(private val enstoreDao: EnstoreDao) {
    ...
    fun addProduct(product: ProductEntity) = executorService.execute{enstoreDao.addProduct(product)}
    fun updateProduct(product: ProductEntity) = executorService.execute{enstoreDao.updateProduct(product)}
    fun deleteProduct(product: ProductEntity) = executorService.execute{enstoreDao.deleteProduct(product)}
